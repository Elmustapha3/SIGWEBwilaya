...
$cniupdated = $_GET['cni'];
  ...
  $req="select a.cni , a.prenom, a.nom, a.email, a.password, a.username, a.partager, a.modifier, a.supprimer, b.type, c.postravail from users a, typeuser b , post_travail c where a.idtype=b.idtype and a.idpost=c.idpost and cni = ?;";
...
<?php
   ...
   if(empty($cni) || empty($prenom) || empty($nom) || empty($post) || empty($_POST['usertype']) || empty($password) || empty($username)){
        ...
        $postdb2= $con->query($postsql2);
            ...
            $sql = "update users set cni = ?, prenom = ?, nom = ?,email = ?,username = ?,password = ?,idtype =?,idpost = ?,partager = ?,modifier = ?,supprimer = ? where cni = ?;";
            $stmt= $con->prepare($sql);
            $stmt->execute([$cni, $prenom, $nom ,$email, $username, $password,$idtypes, $idposts,$perpar,$permod,$persup,$cniupdated]);
