...
function addMapLayerList_spQry() {
    $(document).ready(function () {
        $.ajax({
            type: "GET",
            url: "http://localhost/geoserver/wfs?request=getCapabilities",
            dataType: "xml",
...
$(function () {
    document.getElementById("selectLayer").onchange = function () {
        ...
        $(document).ready(function () {
            $.ajax({
                type: "GET",
                url: "http://localhost/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" + value_layer,
                dataType: "xml",
                success: function (xml) {
    ...
    document.getElementById('attQryRun').onclick = function () {
        ...
        if (layer.options.selectedIndex == 0) {
            ...
            var url = "http://localhost/geoserver/GISSimplified/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" + value_layer + "&CQL_FILTER=" + value_attribute + "+" + value_operator + "+'" + value_txt + "'&outputFormat=application/json"
            // console.log(url);
...
printElement.appendChild(printButton);
    if($("#printButton").hasClass("clicked"))
    {
    windows.print();}
})
*/
 ...
 /* function PrintElem(elem)
{
    ...
    mywindow.document.write(`<html><head><script src="https://cdn.jsdelivr.net/gh/openlayers/openlayers.github.io@master/en/v6.14.1/build/ol.js"></script>
    ...
    <link rel="shortcut icon" href="resources\images\map.png"><title>' + document.title  + '</title>`);
